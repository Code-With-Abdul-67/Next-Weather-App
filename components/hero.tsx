...
const fetchWeatherData = async (city: string): Promise<WeatherData | null> => {
  ...
  try {
    ...
    if (error.response && error.response.status === 401) {
      console.error('Invalid API key. Please verify your OPENWEATHERMAP_API_KEY in .env.local or regenerate it.');
    }
    return null;
  }
};

export const Hero = async ({ city = 'Madrid' }: HeroProps) => {
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 text-center">
          ...
          <div className="w-16 h-16 mx-auto relative">
            <svg className="w-full h-full" viewBox="0 0 100 100">
              ...
              <text x="50" y="50" textAnchor="middle" dy="0.3em" fontSize="10" fill="#fff">N</text>
              <text x="50" y="95" textAnchor="middle" dy="0.3em" fontSize="10" fill="#fff">S</text>
              <text x="5" y="50" textAnchor="middle" dx="-0.3em" fontSize="10" fill="#fff">W</text>
              <text x="95" y="50" textAnchor="middle" dx="0.3em" fontSize="10" fill="#fff">E</text>
              <text x="50" y="40" textAnchor="middle" fontSize="12" fill="#fff">{windSpeed} m/s</text>
            </svg>
          </div>
        </div>
