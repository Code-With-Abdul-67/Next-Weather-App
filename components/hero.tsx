...
const fetchWeatherData = async (city: string): Promise<WeatherData | null> => {
  ...
  if (!apiKey) {
    console.error('API key not set in .env.local. Please add OPENWEATHERMAP_API_KEY to .env.local.');
    return null;
  }

  try {
    console.log('API Request URL:', `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}&units=metric`);
...
export const Hero = async ({ city = 'Karachi' }: HeroProps) => {
  ...
  const description = weatherData ? weatherData.weather[0].description : 'Clear Sky (API Error)';
  ...
  const visibility = weatherData ? Math.round(weatherData.visibility / 1000) : 10; // Convert to km
  ...
  const lowHigh = weatherData ? `Low: ${Math.round(temperature - 4)}° High: ${Math.round(temperature + 2)}°` : 'Low: 6° High: 12°';
  ...
  return (
    <main className="flex flex-col items-center justify-start bg-black min-h-screen text-white p-6">
      ...
      <div className="w-full max-w-4xl bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 text-center mb-6">
        <p className="text-sm text-gray-400 mb-2">Tuesday 05:06 PM PKT, May 27, 2025</p>
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 text-center">
          ...
          <div className="w-20 h-20 mx-auto relative"> {/* Increased from w-16 h-16 to w-20 h-20 */}
            <svg className="w-full h-full" viewBox="0 0 100 100">
              <circle cx="50" cy="50" r="45" fill="none" stroke="#4B5563" strokeWidth="2" />
              <line x1="50" y1="5" x2="50" y2="20" stroke="#fff" strokeWidth="2" transform="rotate(90 50 50)" />
              <text x="50" y="50" textAnchor="middle" dy="0.3em" fontSize="10" fill="#fff" fontWeight="bold">N</text>
              <text x="50" y="95" textAnchor="middle" dy="0.3em" fontSize="10" fill="#fff" fontWeight="bold">S</text>
              <text x="5" y="50" textAnchor="middle" dx="-0.3em" fontSize="10" fill="#fff" fontWeight="bold">W</text>
              <text x="95" y="50" textAnchor="middle" dx="0.3em" fontSize="10" fill="#fff" fontWeight="bold">E</text>
              <text x="50" y="40" textAnchor="middle" fontSize="12" fill="#fff" fontWeight="bold">{windSpeed} m/s</text>
            </svg>
          </div>
        </div>
        ...
        <div className="bg/white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4">
          ...
          <p className="text-sm text-gray-400">Latest UN population data for {cityName}.</p>
        </div>
